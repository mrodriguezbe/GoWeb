...
import (
	...
	"goweb/clase4/practica1/platform/web/request"
	"goweb/clase4/practica1/platform/web/response"
	"net/http"
	"os"
	"strconv"
...
func (d *DefaultProductService) AddNewProductHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var t string = r.Header.Get("AccessToken")

		if t != getToken() {
			fmt.Println("Bad access token")
			response.Error(w, http.StatusUnauthorized, "Bad access token")
			return
		}
...
func (d *DefaultProductService) UpdatePartialHandler() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		var t string = r.Header.Get("Access Token")
